...
const VettingPage = () => {
  ...
  try {
   const response = await axios.put(`http://localhost:7000/api/faculty/review-question/${id}`, {
  status,
  remarks,
}, {
  headers: {
    ...
    console.log("Question accepted successfully:", response.data);
    setViewModalOpen(false); // Close modal after success
  } catch (error) {
...
const handleReject = async (question) => {
  ...
  headers: {
    ...
    console.log("Question rejected successfully:", response.data);
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-30 bg-black opacity-30 md:hidden" onClick={() => setSidebarOpen(false)}></div>
      )}
            ...
            <div className="absolute top-0 left-0 w-full h-full bg-white z-10 shadow-lg rounded-lg p-6 overflow-y-auto">
              <div className="flex flex-col md:flex-row gap-6 h-full border border-black rounded-md">
                <div className="flex-1 rounded-lg flex flex-col gap-6 bg-white p-6">
                  ...
                  <p><strong>Unit:</strong><br />{selectedQuestion.unit}</p>
                  <p><strong>Topic:</strong><br />{selectedQuestion.topic}</p>
                  <p><strong>Marks:</strong><br />{selectedQuestion.mark}</p>
                  <p><strong>Question:</strong><br />{selectedQuestion.question}</p>
                  <p><strong>Answer:</strong><br />{selectedQuestion.answer}</p>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-between border-l border-black p-6">
                  <div>
                    ...
                    <div className="text-sm font-semibold my-1 mb-4">FACULTY ID : {facultyId}</div>
                    ...
                    <div className='flex flex-row items-center justify-around'>
                      <button
                        ...
                        onClick={() => handleAccept(selectedQuestion)}
                      >
                        Accept
                      </button>
                      <button
